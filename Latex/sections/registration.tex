...
\begin{figure*}
	...
	\caption{Illustration of the Hesse- and polygon projection distance. The point p (red) gets projected onto the infinitley extending global plane. Both the point projection (green) and the global planes convex hull (blue) get projected into 2D space (grey). The Hesse distance, i.e. the shortest distance to the infinitley extending plane (red), is shown as well as the minimum distance from the 2D point projection to the polygon (purple).}
	\label{fig:proj}
\end{figure*}
...
For each globaly extracted plane, consider adding a correspondence for all points that are close to that plane.
To do this, we employ two distance models to represent the distance from a point to a plane.

The Hesse-normal form describes the distance from the $i$-th transformed point $T(\vec{p}_i)$ to the ever extending, infinetly large $k$-th plane in 3D vector space
\begin{equation}\label{eq:hesse}
	D_{h}^{i,k} = \vec{n}_{\mathcal{P}_k} \cdot \left[ T(\vec{p}_i) - \vec{a}_{\mathcal{P}_k} \right] ,
\end{equation}
with normal $\vec{n}_{\mathcal{P}_k}$ and receiving point $\vec{a}_{\mathcal{P}_k}$ of the $k$-th plane.
...
Therefore, the line segment is parallel to the normal vector of the plane by definition.
...
\begin{equation}\label{eq:projection}
	\widetilde{T}(\vec{p}_i) = T(\vec{p}_i) - D_{h}^{i,k} \cdot \vec{n}_{\mathcal{P}_k}
\end{equation}

The simplicity of the Hesse distance is at contrast with its inability to represent the extent of the plane.
It becomes obvious that another distance model, based on the polygonic nature of the plane, is required: the polygon projection distance (PPD).
The convex hull is able of representing the expansion of the plane in all directions, and thus is utilized as a distance model.
First, the corresponding point gets projected onto the ever extending, infinetly large plane representation given by the Hesse form.
Keeping track of the points projection $\widetilde{T}(\vec{p}_k)$ from eq. \ref{eq:projection}, i.e. the green point from figure \ref{fig:proj}, is essential for calculating a points distance to a polygon in 3D.
It represents the points "shadow" onto the same plane where the polygon lives in. 
Then, the 3D polygon, i.e. the points that make up the convex hull, as well as the corresponding 3D point projection, are projected again into a 2D vector space, using the direction of the plane.
...
Sunday \cite{pga} presents an optimal algorithm to check wether the point projection lies inside the polygon in 2D. 
...
We setup a distance function, which measures the distance from the query point $p_i$ to an arbitrary point on the line segment.
\begin{equation}
	\label{eq:dist}
...
Now, we say that a point is "close" to a plane, if the Hesse distance is smaller than $\epsilon_H$ and the PPD is smaller than $\epsilon_P$. 
Depending on the threshold values, some points are contemplable having correspondences with multiple planes, if both distances fall below their respective threshold. \todo{@Fabi add actual $\epsilon$ values to the experiments}. 
We then chose to establish a correspondence to the plane with the smallest PPD.
If there are still ambiguities, i.e. multiple PPD's with value zero, a correspondence is established with the plane having the smallest PPD and smallest Hesse distance. 
