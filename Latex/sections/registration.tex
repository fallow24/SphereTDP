...
Therefore, a simple algorithm is sufficient:
For each globally extracted plane, consider adding a correspondence for all points that are close to that plane.
...
We consider a point as ``close'' to a plane, if the Hesse distance is smaller than some threshold $\epsilon_H$ and the PPD is additionally smaller than some other threshold $\epsilon_P$. 
Depending on the threshold values, some points might have correspondences with multiple planes, if both distances fall below their respective threshold. \todo{@Fabi add actual $\epsilon$ values to the experiments}. 
We then choose to establish a correspondence to the plane with the smallest PPD.
