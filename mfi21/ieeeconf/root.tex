...
\paragraph*{1. Real Time Operations}The operation of the robot does not allow post-calculation of the pose. This implies the usage of the onboard computational power.
...
It is easy to calculate and adapt, but due to the number of pre-assumptions, as it makes assumptions about the dynamics of the system, it still delivers reliable data which takes physical behavior into account. 
It is real-time capable and does not rely on delay steps.
...
As we do not use a magentometer, we can not use the gyro bias estimation of the Madgwick filter.
...
Therefore the gyroscope functions as high-pass and the accelerometer as low-pass part of the basic compelemntary filter \cite{min2015complementary}.
...
Let $a$ be the real acceleration and $\epsilon_a$ the error of the measurment, then
\begin{equation}
\iint (a+\epsilon_a) = \iint a+  \iint \epsilon_a = \frac{1}{2}a\cdot t^2+  \frac{1}{2} \epsilon_a \cdot t^2 \,,
\end{equation}
shows that the position depends on $\epsilon_a$ exponentially.
